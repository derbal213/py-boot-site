...
"""
    ...
    def test_extract_single_link(self):
        matches = extract_markdown_links("[one](url1)")
        self.assertListEqual([("one", "url1")], matches)

    def test_extract_spaced_txt_image(self):
        matches = extract_markdown_images("![my cool image](http://img.com/cool.png)")
        self.assertListEqual([("my cool image", "http://img.com/cool.png")], matches)
    
    def test_extract_no_images(self):
        matches = extract_markdown_images("This text has no markdown image")
        self.assertListEqual([], matches)

    def test_extract_img_in_sentence(self):
        matches = extract_markdown_images("Check this out (![wow](url)).")
        self.assertListEqual([("wow", "url")], matches)

    def test_extract_img_empty_txt(self):
        matches = extract_markdown_images("![](http://example.com/no-alt.png)")
        self.assertListEqual([("", "http://example.com/no-alt.png")], matches)
    ...
    def test_extract_spaced_txt_link(self):
        matches = extract_markdown_links("[my cool link](http://img.com/cool.png)")
        self.assertListEqual([("my cool link", "http://img.com/cool.png")], matches)
    ...
    def test_extract_link_in_sentence(self):
        matches = extract_markdown_links("Check this out ([wow](url)).")
        self.assertListEqual([("wow", "url")], matches)

    def test_extract_link_empty_txt(self):
        matches = extract_markdown_links("[](http://example.com/no-alt.png)")
        self.assertListEqual([("", "http://example.com/no-alt.png")], matches)

    def test_extract_link_empty_url(self):
        matches = extract_markdown_links("[alt]()")
        self.assertListEqual([("alt", "")], matches)
